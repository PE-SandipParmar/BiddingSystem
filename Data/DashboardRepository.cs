            ...
            try
            {
                var sql = @"
                    SELECT 
                        ...
                        (SELECT COUNT(*) FROM RefundPayments WHERE RefundStatus = 'Pending' AND YEAR(InitiatedAt) = @Year) as PendingRefunds,
                        ...
                        (SELECT COUNT(*) FROM RefundPayments WHERE RefundStatus = 'Completed' AND YEAR(InitiatedAt) = @Year) as ProcessedRefunds,
                        (SELECT ISNULL(SUM(RefundAmount), 0) FROM RefundPayments WHERE RefundStatus = 'Completed' AND YEAR(InitiatedAt) = @Year) as TotalRefundAmount";
